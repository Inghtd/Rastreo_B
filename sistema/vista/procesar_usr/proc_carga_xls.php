...
if (copy($archivo_temporal,$ruta.$archivo_nombre)){
  ...
  for ($row = 2; $row <= $highestRow; $row++){
    ...
    $carnet=trim(strtoupper($sheet->getCell("A".$row)->getValue()));
    $nombre=trim($sheet->getCell("B".$row)->getValue());
    $unidad_admin=trim($sheet->getCell("C".$row)->getValue());
    $codigo_unidad=$sheet->getCell("D".$row)->getValue();
    $perfil=$sheet->getCell("E".$row)->getValue();
    $agencia=$sheet->getCell("F".$row)->getValue();
    $codigo_dep=$sheet->getCell("G".$row)->getValue();
    $puesto    = trim(strtoupper($sheet->getCell("H".$row)->getValue()));
